...
unsigned short int buf_ent[1]; /*16 bit buffer to generate random bytes*/
...
static ssize_t my_read (struct file *pfile, char __user *pbuffer, size_t len, loff_t *offset){
	...
	*buf_ent=*buf_ent & (0x03ff);
	if(copy_to_user(pbuffer, buf_ent,2)) printk(KERN_INFO "All data could not be sent\n");
	return 0;
}
