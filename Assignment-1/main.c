...
unsigned short int buf_ent[3]; /*16 bit buffer to generate random bytes*/
...
static ssize_t my_read (struct file *pfile, char __user *pbuffer, size_t len, loff_t *offset){
	...
	get_random_bytes(buf_ent, 6);
	*buf_ent=*buf_ent & (0x03ff);		/*16 bits for x axis*/
	*(buf_ent+1)=*(buf_ent+1) & (0x03ff);	/*16 bits for y axis*/
	*(buf_ent+2)=*(buf_ent+2) & (0x03ff);   /*16 bits for z axis*/
	if(copy_to_user(pbuffer, buf_ent,6)) printk(KERN_INFO "Could not be sent\n");
	return 0;
}
